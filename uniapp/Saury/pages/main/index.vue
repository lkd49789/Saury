...
<template>
  <view class="body">
    <view class="swiper-view" v-if="{{GetPersonAnnualCounts.length}}">
      <swiper indicator-dots="{{indicatorDots}}" autoplay="{{autoplay}}" interval="{{interval}}" duration="{{duration}}" circular="{{circular}}" next-margin="40rpx" previous-margin="40rpx" bindchange="bindchange" style="height:405rpx">
          <swiper-item v-for="(item,index) in GetPersonAnnualCounts" :key="index">
    ...
    <view class="function">
      <view class="auditFuntion">
        <swiper indicator-dots="{{indicatorDots}}" autoplay="{{autoplay}}" interval="{{interval}}" duration="{{duration}}" next-margin="50rpx"  style="height:200rpx" display-multiple-items='3'>
            <swiper-item v-for="(item,index) in auditFunction" :key="index">
      ...
      <view class="Notifications" @tap="toMessagePage">
        <view class="tishi" v-if="{{GetUserNotifications_data.length!==0}}">
          <icon class="iconfont icon-tishi" style="font-size:70rpx;color:#ff9900"></icon>
        </view>
        <swiper autoplay interval="3000" circular vertical="{{true}}" skip-hidden-item-layout="{{true}}" display-multiple-items="{{GetUserNotifications_data.length==1?1:2}}" style="height:{{GetUserNotifications_data.length==1?41:82}}rpx;width:86%;" v-if="{{GetUserNotifications_data.length!==0}}">
            <swiper-item catchtouchmove="true" v-for="(item,index) in GetUserNotifications_data" :key="index">
        ...
        <view class="notData" v-else='{{GetUserNotifications_data.length !== 0}}'>
          <!-- <icon class="iconfont icon-tishi" style="font-size:70rpx;color:#ff9900"></icon> -->
